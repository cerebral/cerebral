...
describe('test helpers', () => {
  ...
  describe('runAction', () => {
    it('should test an action that gets state', () => {
      ...
      return runAction(testAction, {state: {foo: 'bar'}}).then(({output}) =>
    ...
    it('should test an action that gets props', () => {
      ...
      return runAction(testAction, {props: {foo: 'bar'}}).then(({output}) =>
        assert.equal(output.foo, 'bar')
      )
    })
    it('should test an action that sets state', () => {
      ...
      return runAction(testAction, {state: {foo: 'bar'}}).then(({state}) =>
      ...
      const runSignal = RunSignal({
        modules: {
          test: {
            ...
            signals: {
              baz: [
                ({state}) => state.set('test.foo', state.get('test.foo') + 1)
              ]
            }
          }
        }
