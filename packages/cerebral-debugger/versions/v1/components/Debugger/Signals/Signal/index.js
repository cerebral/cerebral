...
class Signal extends React.Component {
  ...
  renderOutputs(action) {
    return Object.keys(action.outputs).map((output, index) => {
      const nextFuncIndex = (
        Array.isArray(action.outputs[output][0]) ?
          action.outputs[output][0][0] && action.outputs[output][0][0].functionIndex
        :
          action.outputs[output][0] && action.outputs[output][0].functionIndex
      );
      const isOutput = Boolean(this.props.signal.functionsRun[nextFuncIndex]);
