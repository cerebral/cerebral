...
describe('MobxController', () => {
  ...
  it('should allow changing state through sequences', () => {
    const rootModule = Module({
      model: types.model({
        foo: types.string,
      }),
    ...
    controller.provide().sequences.test()
    assert.equal(controller.provide().store.foo, 'bar2')
  })
  it('should provide store and sequences', () => {
    const rootModule = Module({
      model: types.model({
        foo: types.string,
      }),
    ...
    assert.ok(typeof controller.provide().sequences.test === 'function')
