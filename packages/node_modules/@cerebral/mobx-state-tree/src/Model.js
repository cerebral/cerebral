import { protect, types, unprotect } from 'mobx-state-tree'
...
class Model {
  constructor(controller) {
    const state = extractModulesProp(controller.module.modules, 'state')
    ...
    this.state = types
      .model(
        extractModulesProp(
          ...
          'model',
          (model, module) => {
            const mobxModel = types.model(model || {})

            if (module.views) {
              return mobxModel.views(module.views)
            }
  ...
  updateIn(path, cb, forceChildPathUpdates = false) {
    ...
    path.reduce((currentState, key, index) => {
      if (index === path.length - 1) {
        const currentValue = currentState instanceof ObservableMap
