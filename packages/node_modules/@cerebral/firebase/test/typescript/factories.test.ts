...
import { props, signals, state, string } from 'cerebral/tags'
...
export const sequence: Sequence = [
  ...
  firebase.onChildAdded(props`foo.bar`, signals`some.signal`, onChildOptions),
  firebase.onChildAdded(props`foo.bar`, signals`some.signal`),
  firebase.onChildChanged(
    'some.fixed.path',
    signals`some.signal`,
    onChildOptions
  ),
  firebase.onChildChanged('some.fixed.path', signals`some.signal`),
  firebase.onChildRemoved(
    props`some.dynamic.path`,
    signals`some.signal`,
    onChildOptions
  ),
  firebase.onChildRemoved(props`some.dynamic.path`, signals`some.signal`),
  firebase.onValue('some.path', signals`some.signal`, onValueOptions),
  firebase.onValue('some.path', signals`some.signal`),
