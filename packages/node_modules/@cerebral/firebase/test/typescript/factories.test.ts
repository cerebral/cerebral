...
import { props, sequences, state, string } from 'cerebral/tags'
import * as firebase from '../../factories'
...
export const sequence: Sequence = [
  ...
  firebase.onChildAdded(props`foo.bar`, sequences`some.signal`, onChildOptions),
  firebase.onChildAdded(props`foo.bar`, sequences`some.signal`),
  firebase.onChildChanged(
    'some.fixed.path',
    sequences`some.signal`,
    onChildOptions
  ),
  firebase.onChildChanged('some.fixed.path', sequences`some.signal`),
  firebase.onChildRemoved(
    props`some.dynamic.path`,
    sequences`some.signal`,
    onChildOptions
  ),
  firebase.onChildRemoved(props`some.dynamic.path`, sequences`some.signal`),
  firebase.onValue('some.path', sequences`some.signal`, onValueOptions),
  firebase.onValue('some.path', sequences`some.signal`),
