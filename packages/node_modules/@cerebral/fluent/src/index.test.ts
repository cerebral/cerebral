/* eslint-env mocha */
import { Controller, Module, Dictionary, SequenceFactory, SequenceWithPropsFactory, ComputedValue, Computed } from './';
...
describe('Fluent', () => {
	...
	it('should allow state changes in actions', () => {
		...
		const SequenceWithProps = (<TProps, TOutput = TProps>() => SequenceWithPropsFactory<Context, TProps, TOutput>())();
	...
	it('should allow computed', () => {
		const Sequence = (<TOutput = {}>() => SequenceFactory<Context, TOutput>())();
		...
		let hasUpdated = 0
		const computed = function getter(state: State) {
	...
	it('should send debugging data on mutation methods', () => {
		type State = {
			...
			splice: string[],
			unshift: string[],
			observableMapSet: Dictionary<string>
		}
