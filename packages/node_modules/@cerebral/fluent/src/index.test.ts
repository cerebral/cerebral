/* eslint-env mocha */
import { Controller, Module, ObservableMap, Sequence, SequenceWithProps, Signal, ComputedValue, Computed } from './';
  ...
  it('should allow computed', () => {
    ...
    const rootModule = Module<State, Signals>({
      state: {
			...
			signals: {
				test: Sequence<Context>(s => s.action(function test ({ state }) {
					state.foo = 'bar2'
				}))
			}
    })
