...
import { FluentInstantiatedModuleObjectDefinition } from './types'
...
class Model extends BaseModel {
  constructor(controller: BaseControllerClass) {
    ...
    this.state = extractModuleProp(
      controller.module,
      'state',
      (state: any, module: FluentInstantiatedModuleObjectDefinition) => {
        const newState = this.observeState(state)

        if (module.computed) {
          Object.keys(module.computed).forEach(key => {
            if (!module.computed || !module.computed[key]) {
              return
            }
