...
export default function connect<Props, State, Signals, EP>(
  dependencies: (
    connect: { props: Props; state: State; sequences: Signals }
  ) => EP,
      ...
      with: function<EP>(
        ...
        return {
          ...
          toClass: function(
            ...
            return connect<TProps, TState, TSignals, EP>(
              con as any,
              passedComponent(fakeProps as any)
            )
          },
        }
      },
...
export function ModuleConnectFactory<TState, TSignals>() {
  function To<TModuleState, TModuleSignals>(
    stateLookup: (state: TState) => TModuleState,
    signalsLookup: (state: TSignals) => TModuleSignals
  ) {
    return function returned<TProps>() {
      return {
        with: function<EP>(
          connector: (
            connect: Connector<TProps, TState, TSignals> &
              ModuleConnector<TModuleState, TModuleSignals>
          ) => EP
        ) {
          return {
            to: function(
              ...
              var con = getConnector<
                ...
                TModuleState,
                TModuleSignals
              >(connector, { stateLookup, signalsLookup })
              return connect<TProps, TModuleState, TModuleSignals, EP>(
                con as any,
                passedComponent
              )
            },
...
function getFakeProps() {
  var handler = {
    get: function(target: any, name: any) {
      throw new CerebralError(
        'The props lambda parameter should only be used to conveniently get the type of the components generic props parameter. Use `this.props` in the component instead.'
      )
    },
  }
    ...
    con = (x: Connector<TProps, TState, TSignals>): EP => {
      return connector({
        ...
        module: {
          ...
          sequences: (lookups as any).sequencesLookup(x.sequences),
        },
        props: x.props,
      })
    }
