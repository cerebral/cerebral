...
> extends BaseControllerClass {
  ...
  constructor(
    ...
    this.sequences = extractModuleProp(
      this.module,
      'sequences',
      (sequences: any, module: any) => {
        return Object.keys(sequences).reduce(
          (runablesequences: any, key: string) => {
  ...
  addModule(path: string, module: ModuleClass) {
    ...
    updateIn(
      ...
      pathArray.concat(moduleKey),
      (parentState: { [key: string]: any }, key: string) => {
        parentState[key] = Object.keys(newModule.sequences).reduce(
          (sequences: any, key: string) => {
