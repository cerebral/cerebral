...
describe('Angular', () => {
  describe(`Component`, () => {
    ...
    before(() => {
      TestBed.configureTestingModule({
        ...
        providers: [
          ChangeDetectorRef,
          {
            provide: AppService,
            useFactory: () => {
              return new AppService(initialStore)
            },
          },
        ],
      }).compileComponents()
      ...
      controller = TestBed.get(AppService)
    ...
    it('should be able to expose host component state', () => {
      fixture.detectChanges()
      assert.strictEqual(count.value, 108)
    })

    it('should be able to expose subComponent state', () => {
      fixture.detectChanges()
      assert.strictEqual(subCmp.componentInstance.name, 'TestSubComponent')
      assert.strictEqual(subCmp.componentInstance.subCompObj.freq, 1)
      assert.strictEqual(subCmp.componentInstance.subCompObj.id, 123)
      assert.strictEqual(subCmp.componentInstance.subCompObj.name, 'cerebral')
    })

    it('should be able to expose signal', () => {
      assert.strictEqual(typeof cmp.increase === 'function', true)
      assert.strictEqual(typeof cmp.decrease === 'function', true)
      assert.strictEqual(typeof cmp.increaseFreq === 'function', true)
      assert.strictEqual(typeof cmp.changeSubCompName === 'function', true)
    })

    it('should update state when signal is emitted', () => {
      increaseClick.click()
      assert.strictEqual(controller.getState('count'), 109)
      decreaseClick.click()
      assert.strictEqual(controller.getState('count'), 108)
      increaseFreqClick.click()
      const subComp = controller.getState('subCompObj')
      assert.strictEqual(subComp.freq, 2)
      assert.strictEqual(subComp.name, 'cerebral')
    })
    ...
    it('should update view when signal is emitted', () => {
      ...
      increaseClick.click()
      fixture.detectChanges()
      assert.strictEqual(count.value, 110)
    })
    ...
    it('should update subComponent view when signal is emitted', () => {
      ...
      fixture.detectChanges()
      assert.strictEqual(subCmp.componentInstance.subCompObj.freq, 3)
      assert.strictEqual(subCmp.componentInstance.subCompObj.name, 'cerebralTest')
    })
  })
})
*/
