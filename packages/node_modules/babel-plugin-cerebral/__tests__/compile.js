...
describe('Transform dot syntax from `statePath` to string', () => {
  it('should work for simple dot usage', () => {
    const code = `
      import {statePath} from 'cerebral/proxy';
      statePath.hello.world;
    `
    const { code: result } = transform(code, pluginOptions)
    expect(result).toMatchSnapshot()
  })
  ...
  it('should work with nested usage', () => {
    const code = `
      import {statePath} from 'cerebral/proxy';
      statePath.hello[statePath.world];
    `
    const { code: result } = transform(code, pluginOptions)
    expect(result).toMatchSnapshot()
  })
  ...
  it('should work with other tags', () => {
    const code = `
      import {statePath, state} from 'cerebral/proxy';
      statePath.hello[state.world];
    `
    const { code: result } = transform(code, pluginOptions)
    expect(result).toMatchSnapshot()
  })
