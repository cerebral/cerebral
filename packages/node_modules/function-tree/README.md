...
## provider
A provider is automatically wrapped when using the Provider factory.
...
const myProvider2 = Provider((context) => ({
  foo() {
    return context.otherProvider.foo()
  }
}))
...
Providers lets us do some pretty amazing things. The debugger for **function-tree** is actually just a provider that sends information to the debugger about execution.
