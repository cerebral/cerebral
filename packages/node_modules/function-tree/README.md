...
```js
function myFunctionFactory(someTag) {
  function myFunction ({resolve}) {
    // Evaluates to the tags value, if not a tag it will just return the value
    resolve.value(someTag)

    // Evaluates the path in the tag
    resolve.path(someTag)

    // Evaluates if variable is a tag, can optionally pass array of types
    // ['props', 'string'] as second argument
    resolve.isTag(someTag)

    // Evaluates if variable is a ContextualValue. Useful when you have created custom
    // stuff to be evaluated in tags or other, extending ContextualValue class
    resolve.isContextualValue(someTag)
  }
...
```js
import {createTemplateTag} from 'function-tree/tags'
...
Converts an object passed to a function factory, resolving any template tags.

```js
import {convertObjectWithTemplates} from 'function-tree/tags'

function myFunctionFactory(someObject) {
  function myFunction({resolve}) {
    const object = convertObjectWithTemplates(someObject, resolve)
  }
}
```
...
class Uppercaser extends ContextualValue {
  constructor(tag) {
    super()
    this.tag = tag
  }
