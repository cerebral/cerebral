...
describe('Tags', () => {
  ...
  it('should return path', () => {
    const tag = props`foo.bar`
    const propsObject = {}
    assert.strictEqual(tag.getPath({ props: propsObject }), 'foo.bar')
  })
  it('should return dynamic path', () => {
    const tag = props`foo.${props`bar`}`
    const propsObject = { bar: 'baz' }
    assert.strictEqual(tag.getPath({ props: propsObject }), 'foo.baz')
  })
  it('should return all tags', () => {
    ...
    assert.strictEqual(tags[0].type, 'props')
    assert.strictEqual(tags[0].getPath(getters), 'foo.baz')
    assert.strictEqual(tags[0].getValue(getters), 'mip')
    assert.strictEqual(tags[1].type, 'props')
    assert.strictEqual(tags[1].getPath(getters), 'bar')
    assert.strictEqual(tags[1].getValue(getters), 'baz')
  })
  it('should have string representation', () => {
    const tag = props`foo.${props`bar`}`
    // eslint-disable-next-line no-template-curly-in-string
    assert.strictEqual(tag.toString(), 'props`foo.${props`bar`}`')
  })
  it('should have string representation of path containing a zero number', () => {
    const tag = props`foo.${0}`
    // eslint-disable-next-line no-template-curly-in-string
    assert.strictEqual(tag.toString(), 'props`foo.0`')
  })
})
