...
type BranchContext<TPaths = {}, TProps = {}> = IBranchContext<TPaths, TProps> & ProviderContext;
...
describe('Fluent', () => {
	...
	it('should return paths correctly', () => {
		...
		var seq = (SequenceWithProps<Props>((x) =>
			x.action(action1).branch(action3).paths({
	...
	describe('inference', () => {
		...
		it('should infer props on paths', () => {
			function action1(context: BranchContext<{ pathA: { foo: string }; pathB: { bar: number } }>) {
				if (Math.random()) {
