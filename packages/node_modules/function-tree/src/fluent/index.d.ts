...
export interface IContextWithPaths<TProps, TPaths> extends IContext<TProps> {
    path: {
        [key in keyof TPaths]: (input: TPaths[key]) => TPaths[key];
    };
}
...
export declare class SequenceBuilder<TContext = {}, TProps = {}> {
    ...
    pathsAction<TOutput, TPaths>(action: ((input: TContext & IContextWithPaths<TProps, TPaths>) => TOutput)): {
        paths: (paths: {
            [key in keyof TPaths]: (input: SequenceBuilder<TContext, TProps & TPaths[key]>) => SequenceBuilder<TContext, TProps & TPaths[key]>;
...
export declare function sequenceFactory<TContext, TProps>(arg: (input: SequenceBuilder<TContext, TProps>) => SequenceBuilder<TContext, TProps>): Primitive;
