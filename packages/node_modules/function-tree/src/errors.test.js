...
describe('errors', () => {
  ...
  it('should have extended execution error', () => {
    ...
    assert.strictEqual(error.toJSON().message, 'foo')
    assert.deepStrictEqual(error.toJSON().execution, { name: 'exec' })
    assert.deepStrictEqual(error.toJSON().funcDetails, {
      name: 'func',
      functionIndex: 1,
    })
    assert.deepStrictEqual(error.toJSON().payload, { foo: 'bar' })
    assert.ok(error.toJSON().stack)
  })
})
