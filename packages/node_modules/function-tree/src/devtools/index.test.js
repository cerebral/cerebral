...
describe('Devtools', () => {
  ...
  it('should add function tree', done => {
    ...
    setTimeout(() => {
      ...
      assert.equal(Object.keys(ft.contextProviders).length, 1)
      devtools.add(ft)
      assert.equal(Object.keys(ft.contextProviders).length, 2)
  ...
  it('should remove all trees', done => {
    ...
    setTimeout(() => {
      ...
      ftA.type = 'A'
      devtools.add(ftA)
      assert.equal(Object.keys(ftA.contextProviders).length, 2)
      assert.equal(devtools.trees.length, 1)
      const ftB = new FunctionTree()
      ftB.type = 'B'
      devtools.add(ftB)
      assert.equal(Object.keys(ftB.contextProviders).length, 2)
      assert.equal(devtools.trees.length, 2)
      devtools.removeAll()
      assert.equal(Object.keys(ftA.contextProviders).length, 1)
      assert.equal(Object.keys(ftB.contextProviders).length, 1)
      assert.equal(devtools.trees.length, 0)
      assert.equal(devtools.isConnected, true)
      mockServer.stop(done)
    }, 70)
