...
describe('Devtools', () => {
  ...
  it('should add function tree', done => {
    ...
    setTimeout(() => {
      ...
      assert.equal(Object.keys(ft.contextProviders).length, 3)
      devtools.add(ft)
      assert.equal(Object.keys(ft.contextProviders).length, 4)
  ...
  it('should remove all trees', done => {
    ...
    setTimeout(() => {
      ...
      ftA.type = 'A'
      devtools.add(ftA)
      assert.equal(Object.keys(ftA.contextProviders).length, 4)
      assert.equal(devtools.trees.length, 1)
      const ftB = new FunctionTree()
      ftB.type = 'B'
      devtools.add(ftB)
      assert.equal(Object.keys(ftB.contextProviders).length, 4)
      assert.equal(devtools.trees.length, 2)
      devtools.removeAll()
      assert.equal(Object.keys(ftA.contextProviders).length, 3)
      assert.equal(Object.keys(ftB.contextProviders).length, 3)
      assert.equal(devtools.trees.length, 0)
      assert.equal(devtools.isConnected, true)
      mockServer.stop(done)
    }, 70)
  ...
  it('should watch function tree executions', done => {
    ...
    setTimeout(() => {
      ...
      assert.deepEqual(messages.execution.data.execution.payload, {
        foo: 'bar',
      })
  ...
  it('should keep execution messages when debugger is not ready and send bulk messages after debugger is ready', done => {
    ...
    setTimeout(() => {
      ...
      assert.deepEqual(executionFunctionEndMessage.data.execution.output, {
        bar: 'baz',
      })
