...
class FunctionTreeExecution {
  ...
  */
  createContext(functionDetails, payload, prevPayload) {
    ...
    if (debuggerProvider) {
      return Object.keys(contextProviders).reduce((currentContext, name) => {
        const provider = contextProviders[name]

        if (provider instanceof Provider && provider.wrap) {
          currentContext[name] = debuggerProvider.wrapProvider(
            name,
            context[name]
          )
        } else {
