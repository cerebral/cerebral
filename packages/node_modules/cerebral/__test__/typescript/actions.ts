import { sequence, Action, ActionChain, ActionContext, ActionFactory, Chain, Computed } from '../..'
...
export function someFactory(value1: Tag<string> | string, value2: Computed<number>): Action {
  return function someAction({ resolve, state }: ActionContext) {
    const test1 = resolve.isTag(value1)
    const test2 = resolve.isResolveValue(value1)
    const test3 = resolve.isTag(value2)
    const test4 = resolve.isResolveValue(value2)
