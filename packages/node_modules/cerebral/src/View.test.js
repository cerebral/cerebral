...
import { state, props, moduleState, sequences, computed } from './tags'
...
describe('View', () => {
  ...
  it('should be able to dynamically define sequences', () => {
    const rootModule = Module({
      ...
      sequences: {
        foo: [({ operators }) => operators.set(state`test`, 'fooSequence')],
      },
    })
    const controller = Controller(rootModule)
    const view = new View({
    ...
    assert.equal(controller.getModel().get(['test']), 'fooSequence')
  })
  it('should be able to adjust props with function (mergeProps)', () => {
    ...
    const view = new View({
      ...
      mergeProps: (depsProps, ownProps, get) => {
        ...
        assert.equal(typeof depsProps.fooSequence, 'function')
        assert.deepEqual(ownProps, { mip: 'mop' })
        assert.equal(get(state`foo`), 'bar')
    ...
    view.render(
      ...
      (componentProps) => {
        ...
        assert.equal(componentProps.fooSequence, undefined)
      }
    )
  })
  it('should mount correctly', () => {
    ...
    view.mount()
    assert.deepEqual(view.dependencyMap, { foo: true })
  ...
  it('should update devtools components map and add entity to controller dependency store when mounting', () => {
    ...
    view.mount()
    assert.equal(devtoolsUpdateWatchMapCalled, 1)
  ...
  it('should update devtools components map and remove entity from controller dependency store when unmounting', () => {
    ...
    view.unMount()
    assert.equal(devtoolsUpdateWatchMapCalled, 2)
  ...
  it('should update tags', () => {
    ...
    const rootModule = Module({
      ...
      sequences: {
        test: [
          ({ operators }) => {
            operators.set(state`currentItem`, '1')
          },
        ],
      },
    })
    ...
    const view = new View({
      ...
      onUpdate: (stateChanges, force) => {
        ...
        assert.deepEqual(view.dependencyMap, {
          'list.1': true,
          currentItem: true,
        })
        renderCount++
  ...
  it('should update on props change', () => {
    ...
    assert.deepEqual(view.dependencyMap, { bar: true })
  })
  ...
  it('should rerender on state update', () => {
    ...
    const rootModule = Module({
      ...
      sequences: {
        methodCalled: [({ operators }) => operators.set(state`foo`, 'bar2')],
      },
    })
    const controller = Controller(rootModule)
  ...
  it('should render only ones where multiple state changes', () => {
    const rootModule = Module({
      ...
      sequences: {
        test: [
          ({ operators }) => {
            operators.set(state`foo`, 'bar2')
            operators.set(state`bar`, 'foo2')
          },
        ],
      },
    })
  ...
  it('should rerender on parent dep replacement', () => {
    ...
    const rootModule = Module({
      ...
      sequences: {
        methodCalled: [
          ({ operators }) => operators.set(state`foo`, { bar: 'baz2' }),
        ],
      },
    })
    const controller = Controller(rootModule)
  ...
  it('should register computed dependencies with "get"', () => {
    let renderCount = 0
    const fooComputed = Computed(({ get }) => get(state`foo`))
    const rootModule = Module({
      state: {
        foo: 'bar',
      },
  ...
  it('should register reactions', () => {
    ...
    view.update()
    controller.getSequence('methodCalled')()
    assert.equal(reactionValue, 'bar2baz')
  })
  describe('STRICT render update', () => {
    ...
    it('should be able to override immediate child interest', () => {
      ...
      const rootModule = Module({
        ...
        sequences: {
          methodCalled: [
            ({ operators }) => operators.set(state`foo.0.bar`, 'baz2'),
          ],
        },
      })
      const controller = Controller(rootModule)
    ...
    it('should by default update when nested children update', () => {
      ...
      const rootModule = Module({
        ...
        sequences: {
          methodCalled: [
            ({ operators }) => operators.set(state`foo.bar.baz`, 'value2'),
          ],
        },
      })
      const controller = Controller(rootModule)
    ...
    it('should update when undefined path becoming defined', () => {
      ...
      view.mount()
      assert.equal(renderCount, 0)
      assert.deepEqual(view.dependencyMap, { 'foo.bar.baz': true })
      view.render({}, (componentProps) => {
        assert.equal(componentProps.foo, undefined)
      })
      controller.getSequence('methodCalled')()
    ...
    it('should by default update when nested children update with array', () => {
      ...
      const rootModule = Module({
        ...
        sequences: {
          methodCalled: [
            ({ operators }) => operators.set(state`foo.0.bar`, 'value2'),
          ],
        },
      })
      const controller = Controller(rootModule)
  ...
  describe('Computed', () => {
    it('should allow use of Computed as state dependency and access props', () => {
      const rootModule = Module({
        computed: {
          foo: Computed(({ get }) => {
    ...
    it('should update dependency map when Computed is rerun', () => {
      let renderCount = 0
      const rootModule = Module({
        state: {
          map: {},
        ...
        sequences: {
          changeState: [
            ({ operators }) => operators.set(state`map.1`, { awesome: true }),
          ],
          changeState2: [
            ({ operators }) => operators.set(state`map.1.awesome`, false),
          ],
        },
      })
      const controller = Controller(rootModule)
      ...
      const rootModule = Module({
        ...
        computed: {
          projects: Computed(
            ...
            ({ userProjects, get }) => {
              return userProjects.map((projectKey) =>
        ...
        sequences: {
          changeState: [
            ({ operators }) => operators.push(state`user.projects`, '2'),
          ],
        },
      })
      const controller = Controller(rootModule)
    ...
    it('should handle strict deps', () => {
      ...
      const rootModule = Module({
        ...
        sequences: {
          changeState: [
            ({ operators }) => operators.set(state`projects.1.title`, 'woop'),
          ],
        },
      })
      const controller = Controller(rootModule)
  ...
  it('should handle props updating state deps', () => {
    const rootModule = Module({
      ...
      sequences: {
        changeState: [({ operators }) => operators.set(state`bar`, 'baz2')],
      },
    })
    const controller = Controller(rootModule)
    const view = new View({
  ...
  it('should handle props composition updating value', () => {
    const rootModule = Module({
      ...
      sequences: {
        changeState: [
          ({ operators }) => {
            operators.set(state`field.value`, 'foo')
            operators.merge(state`field`, { mip: 'mop' })
          },
        ],
      },
    })
    ...
    view.mount()
    assert.deepEqual(view.dependencyMap, { 'field.**': true })
    view.render(
      {
        path: 'field',
      },
  ...
  it('should warn if prop is from the Cerebral state tree when initializing', () => {
    ...
    const controller = Controller(rootModule, {
      devtools: {
        warnStateProps: true,
        preventExternaloperatorss: true,
        init() {},
        send() {},
        updateWatchMap() {},
        updateComputedMap() {},
