...
describe('View', () => {
  ...
  it('should be able to extract state', () => {
    ...
    assert.deepStrictEqual(view.controller.getState(), { foo: 'bar' })
    view.render({}, (componentProps) => {
  ...
  it('should be able to adjust props with function (mergeProps)', () => {
    ...
    const view = new View({
      ...
      mergeProps: (depsProps, ownProps, get) => {
        ...
        assert.deepStrictEqual(ownProps, { mip: 'mop' })
  ...
  it('should mount correctly', () => {
    ...
    view.mount()
    assert.deepStrictEqual(view.dependencyMap, { foo: true })
  ...
  it('should update devtools components map and add entity to controller dependency store when mounting', () => {
    ...
    const controller = App(rootModule, {
      devtools: {
        ...
        updateWatchMap(component, nextDeps, prevDeps) {
          assert.deepStrictEqual(nextDeps, { foo: true })
          assert.deepStrictEqual(prevDeps, {})
    ...
    view.controller.dependencyStore.addEntity = function(component, depsMap) {
      assert.deepStrictEqual(depsMap, { foo: true })
      assert.strictEqual(component._displayName, 'Test')
  ...
  it('should update devtools components map and remove entity from controller dependency store when unmounting', () => {
    ...
    view.controller.devtools.updateWatchMap = function(
      ...
      assert.deepStrictEqual(prevDeps, { foo: true })
  ...
  it('should update tags', () => {
    ...
    const view = new View({
      ...
      onUpdate: (stateChanges, force) => {
        ...
        assert.deepStrictEqual(stateChanges[0].path, ['currentItem'])
        assert.strictEqual(force, undefined)
        view.updateFromState(stateChanges, null, force)
        assert.deepStrictEqual(view.dependencyMap, {
          'list.1': true,
          currentItem: true,
        })
        renderCount++
  ...
  it('should update on props change', () => {
    ...
    assert.deepStrictEqual(view.dependencyMap, { bar: true })
  })
  ...
  it('should rerender on state update', () => {
    ...
    const view = new View({
      ...
      onUpdate: (stateChanges, force) => {
        assert.strictEqual(stateChanges.length, 1)
        assert.deepStrictEqual(stateChanges[0].path, ['foo'])
        assert.strictEqual(force, undefined)
        view.updateFromState(stateChanges, null, force)
        renderCount++
      },
  ...
  it('should render only ones where multiple state changes', () => {
    ...
    const view = new View({
      ...
      onUpdate: (stateChanges, force) => {
        ...
        assert.deepStrictEqual(stateChanges[0].path, ['foo'])
        assert.deepStrictEqual(stateChanges[1].path, ['bar'])
        assert.strictEqual(force, undefined)
        renderCount++
      },
    })
  ...
  it('should register dependencies with "get.path"', () => {
    ...
    assert.deepStrictEqual(paths, ['foo', 'poo.par', 'foo.bar.man', 'foo.bong'])
  })
  ...
  describe('STRICT render update', () => {
    ...
    it('should return array', () => {
      ...
      view.mount()
      view.render({}, (componentProps) => {
        assert.deepStrictEqual(componentProps.foo, [])
      })
      assert.deepStrictEqual(view.dependencyMap, { 'foo.*': true })
    })
    ...
    it('should by default update when nested children update', () => {
      ...
      const view = new View({
        ...
        onUpdate: (stateChanges, force) => {
          assert.strictEqual(stateChanges.length, 1)
          assert.deepStrictEqual(stateChanges[0].path, ['foo', 'bar', 'baz'])
          assert.strictEqual(force, undefined)
          renderCount++
        },
      })
    ...
    it('should update when undefined path becoming defined', () => {
      ...
      view.mount()
      assert.strictEqual(renderCount, 0)
      assert.deepStrictEqual(view.dependencyMap, { 'foo.bar.baz': true })
      view.render({}, (componentProps) => {
        assert.strictEqual(componentProps.foo, undefined)
      })
      controller.getSequence('methodCalled')()
    ...
    it('should update when undefined path becoming defined deeply', () => {
      ...
      const view = new View({
        ...
        onUpdate: (stateChanges, force) => {
          assert.strictEqual(stateChanges.length, 1)
          assert.deepStrictEqual(stateChanges[0].path, [
            'scope',
            'scopeid',
            'user',
            'userid',
      ...
      view.render({}, (componentProps) => {
        assert.deepStrictEqual(componentProps.user, undefined)
      })

      controller.getSequence('methodCalled')()
      assert.strictEqual(renderCount, 1)
      view.render({}, (componentProps) => {
        assert.deepStrictEqual(componentProps.user, { userid: 'bong' })
      })
    })
    it('should by default update when nested children update with array', () => {
      ...
      const view = new View({
        ...
        onUpdate: (stateChanges, force) => {
          assert.strictEqual(stateChanges.length, 1)
          assert.deepStrictEqual(stateChanges[0].path, ['foo', '0', 'bar'])
          assert.strictEqual(force, undefined)
          renderCount++
        },
      })
  ...
  describe('Compute', () => {
    ...
    it('should update dependency map when Compute is rerun', () => {
      ...
      const view = new View({
        ...
        onUpdate: (stateChanges, force) => {
          if (!renderCount) {
            assert.strictEqual(stateChanges.length, 1)
            assert.deepStrictEqual(stateChanges[0].path, ['map', '1'])
            assert.strictEqual(force, undefined)
            view.updateFromState(stateChanges, null, force)
            assert.deepStrictEqual(view.dependencyMap, {
              'map.*': true,
            ...
            assert.strictEqual(stateChanges.length, 1)
            assert.deepStrictEqual(stateChanges[0].path, ['map', '1', 'awesome'])
            assert.strictEqual(force, undefined)
            view.updateFromState(stateChanges, null, force)
            assert.deepStrictEqual(view.dependencyMap, {
              'map.*': true,
    ...
    it('should handle complex state changes', () => {
      ...
      const view = new View({
        ...
        onUpdate: (stateChanges, force) => {
          assert.strictEqual(stateChanges.length, 1)
          assert.deepStrictEqual(stateChanges[0].path, ['user', 'projects'])
          assert.strictEqual(force, undefined)
          view.updateFromState(stateChanges, null, force)
          assert.deepStrictEqual(view.dependencyMap, {
            'projects.0': true,
      ...
      view.render({}, (componentProps) => {
        assert.deepStrictEqual(componentProps.projects, ['foo', 'bar'])
      })
      view.controller.getSequence('changeState')()
      view.render({}, (componentProps) => {
        assert.deepStrictEqual(componentProps.projects, ['foo', 'bar', 'baz'])
      })
      assert.strictEqual(renderCount, 1)
    })
    it('should handle strict deps', () => {
      ...
      const view = new View({
        ...
        onUpdate: (stateChanges, force) => {
          assert.strictEqual(stateChanges.length, 1)
          assert.deepStrictEqual(stateChanges[0].path, ['projects', '1', 'title'])
          assert.strictEqual(force, undefined)
          view.updateFromState(stateChanges, null, force)
          renderCount++
        },
      ...
      assert.strictEqual(renderCount, 0)
      view.render({}, (componentProps) => {
        assert.deepStrictEqual(componentProps.projects, [
          { title: '0' },
          { title: '1' },
        ])
      })
  ...
  it('should handle props composition updating value', () => {
    ...
    view.mount()
    assert.deepStrictEqual(view.dependencyMap, { 'field.**': true })
    view.render(
      {
        path: 'field',
      },
      (componentProps) => {
        assert.deepStrictEqual(componentProps.field, {
          value: '',
        })
      }
    )
  ...
  it('should handle computed from controller', () => {
    ...
    view.mount()
    assert.deepStrictEqual(view.dependencyMap, { foo: true, bar: true })
  })
})
