...
import { state, props, moduleState, signals, computed } from './tags'
...
describe('View', () => {
  ...
  it('should be able to dynamically define signals', () => {
    const rootModule = Module({
      ...
      signals: {
        foo: [({ operators }) => operators.set(state`test`, 'fooSignal')],
      },
    })
    const controller = Controller(rootModule)
    const view = new View({
      dependencies: {
        signal: signals`${props`path`}`,
      },
      controller,
      props: {
        path: 'foo',
