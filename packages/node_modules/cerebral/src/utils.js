...
export function copyAndFreezeObject(rootObject) {
  function deepFreeze(val) {
    if (isComplexObject(val)) return deepFreezeObject(val)
    if (Array.isArray(val)) return deepFreezeArray(val)
    return val
  }
  ...
  return deepFreeze(JSON.parse(JSON.stringify(rootObject)))
}
