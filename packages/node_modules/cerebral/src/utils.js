...
export function extractModulesProp(modules, prop, transform) {
  return Object.keys(modules).reduce((extracted, subKey) => {
    if (
      modules[subKey][prop] ||
      Object.keys(modules[subKey].modules || {}).length
    ) {
      extracted[subKey] = {}
    }

    if (Object.keys(modules[subKey].modules || {}).length) {
      extracted[subKey] = extractModulesProp(
        modules[subKey].modules,
        prop,
        transform
      )
    }

    if (modules[subKey][prop]) {
      const newProps = transform
        ? transform(modules[subKey][prop], modules[subKey])
        : modules[subKey][prop]

      Object.assign(extracted[subKey], newProps)
    }
