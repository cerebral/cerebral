...
describe('UniversalApp', () => {
  it('should track state changes', () => {
    ...
    assert.strictDeepEqual(controller.changes, [
  ...
  it('should expose method to get state changes', () => {
    ...
    assert.strictDeepEqual(controller.getChanges(), { foo: 'bar2' })
  })
  ...
  it('sets its hasRun flag to true in getScript()', () => {
    ...
    assert.strictEqual(controller.hasRun, false)
    controller.getScript()
    assert.strictEqual(controller.hasRun, true)
  })
  ...
  it('should run a module sequence', () => {
    ...
    controller.runSequence('app.moduleSequence', { value: 1 }).then(() => {
      assert.strictDeepEqual(controller.getState(), { app: { foo: 'bar2' } })
      assert.strictDeepEqual(controller.changes, [
  ...
  it('should allow multiple runs', () => {
    ...
    assert.strictDeepEqual(controller.getState(), { foo: 'bar4' })
    assert.strictDeepEqual(controller.changes, [
      { path: ['foo'], forceChildPathUpdates: true },
      { path: ['foo'], forceChildPathUpdates: true },
      { path: ['foo'], forceChildPathUpdates: true },
  ...
  it('should update its state', () => {
    ...
    assert.strictDeepEqual(controller.getState(), { foo: newState })
    assert.strictDeepEqual(controller.changes, [
      { path: ['foo'], forceChildPathUpdates: true },
    ])
  })
