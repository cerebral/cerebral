/* eslint-env mocha */
import { Controller, Module } from '../'
import assert from 'assert'
import { increment } from './'
import { props, state, string } from '../tags'

describe('operator.increment', () => {
  test('should increment value in model with value of one by default', () => {
    const rootModule = Module({
      state: {
        foo: 0,
      },
      signals: {
        test: [increment(state`foo`)],
      },
    })
    const controller = Controller(rootModule)
    controller.getSignal('test')()
    assert.deepEqual(controller.getState(), { foo: 1 })
  })
  test('should increment value in model with given value', () => {
    const rootModule = Module({
      state: {
        foo: 0,
      },
      signals: {
        test: [increment(state`foo`, 2)],
      },
    })
    const controller = Controller(rootModule)
    controller.getSignal('test')()
    assert.deepEqual(controller.getState(), { foo: 2 })
  })
  test('should increment value in props with value of one by default', () => {
    const rootModule = Module({
      signals: {
        test: [
          increment(props`foo`),
          ({ props }) => {
            assert.equal(props.foo, 1)
          },
        ],
      },
    })
    const controller = Controller(rootModule)
    controller.getSignal('test')({
      foo: 0,
    })
  })
  test('should increment value in props with given value', () => {
    const rootModule = Module({
      signals: {
        test: [
          increment(props`foo`, 2),
          ({ props }) => {
            assert.equal(props.foo, 2)
          },
        ],
      },
    })
    const controller = Controller(rootModule)
    controller.getSignal('test')({
      foo: 0,
    })
  })
  test('should increment value to model from props', () => {
    const rootModule = Module({
      state: {
        foo: 1,
      },
      signals: {
        test: [increment(state`foo`, props`value`)],
      },
    })
    const controller = Controller(rootModule)
    controller.getSignal('test')({
      value: 1,
    })
    assert.deepEqual(controller.getState(), { foo: 2 })
  })
  test('should increment value to props from model', () => {
    const rootModule = Module({
      state: {
        foo: 2,
      },
      signals: {
        test: [
          increment(props`value`, state`foo`),
          ({ props }) => {
            assert.equal(props.value, 3)
          },
        ],
      },
    })
    const controller = Controller(rootModule)
    controller.getSignal('test')({
      value: 1,
    })
  })
  test('should increment value to props from props', () => {
    const rootModule = Module({
      signals: {
        test: [
          increment(props`value`, props`grabValue`),
          ({ props }) => {
            assert.equal(props.value, 3)
          },
        ],
      },
    })
    const controller = Controller(rootModule)
    controller.getSignal('test')({
      value: 1,
      grabValue: 2,
    })
  })
  test('should increment value to model from model', () => {
    const rootModule = Module({
      state: {
        foo: 1,
        grabValue: 2,
      },
      signals: {
        test: [increment(state`foo`, state`grabValue`)],
      },
    })
    const controller = Controller(rootModule)
    controller.getSignal('test')()
    assert.equal(controller.getState().foo, 3)
  })
  test('should throw on bad target argument', (done) => {
    const rootModule = Module({
      state: {
        foo: 'bar',
      },
      signals: {
        test: [increment(string`foo`)],
      },
    })
    const controller = Controller(rootModule)
    controller.removeListener('error')
    controller.once('error', (error) => {
      assert.ok(error)
      done()
    })

    controller.getSignal('test')()
  })
  test('should throw on bad value argument', (done) => {
    const rootModule = Module({
      state: {
        foo: 0,
      },
      signals: {
        test: [increment(string`foo`, 'bar')],
      },
    })
    const controller = Controller(rootModule)
    controller.removeListener('error')
    controller.once('error', (error) => {
      assert.ok(error)
      done()
    })

    controller.getSignal('test')()
  })
})
