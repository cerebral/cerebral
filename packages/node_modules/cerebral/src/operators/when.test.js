/* eslint-env mocha */
import { Controller, Module } from '../'
import assert from 'assert'
import { when } from './'
import { props, state } from '../tags'

describe('operator.when', () => {
  test('should check truthy value of props', () => {
    let count = 0
    const rootModule = Module({
      signals: {
        test: [
          when(props`foo`),
          {
            true: [
              () => {
                count++
              },
            ],
            false: [],
          },
        ],
      },
    })
    const controller = Controller(rootModule)
    controller.getSignal('test')({ foo: 'bar' })
    assert.equal(count, 1)
  })
  test('should check truthy value of state', () => {
    let count = 0
    const rootModule = Module({
      state: {
        foo: false,
      },
      signals: {
        test: [
          when(state`foo`),
          {
            true: [],
            false: [
              () => {
                count++
              },
            ],
          },
        ],
      },
    })
    const controller = Controller(rootModule)
    controller.getSignal('test')()
    assert.equal(count, 1)
  })
  test('should check truthy props using function', () => {
    let accepted = 0
    let discarded = 0
    const rootModule = Module({
      signals: {
        test: [
          when(props`value`, (value) => Boolean(value.length)),
          {
            true: [
              () => {
                accepted++
              },
            ],
            false: [
              () => {
                discarded++
              },
            ],
          },
        ],
      },
    })
    const controller = Controller(rootModule)
    controller.getSignal('test')({ value: '' })
    controller.getSignal('test')({ value: 'foo' })
    assert.equal(accepted, 1)
    assert.equal(discarded, 1)
  })
  test('should check truthy state using function', () => {
    let discarded = 0
    const rootModule = Module({
      state: {
        foo: 'bar',
      },
      signals: {
        test: [
          when(state`foo`, (value) => value === 'bar'),
          {
            true: [
              ({ state }) => {
                state.set('foo', 'bar2')
              },
            ],
            false: [
              () => {
                discarded++
              },
            ],
          },
        ],
      },
    })
    const controller = Controller(rootModule)
    controller.getSignal('test')()
    controller.getSignal('test')()
    assert.equal(discarded, 1)
  })
  test('should check truthy state using function and multiple values', () => {
    const results = []
    const rootModule = Module({
      state: {
        foo: 'bar',
      },
      signals: {
        test: [
          when(state`foo`, props`bar`, (foo, bar) => foo === bar),
          {
            true: [
              () => {
                results.push('true')
              },
            ],
            false: [
              () => {
                results.push('false')
              },
            ],
          },
        ],
      },
    })
    const controller = Controller(rootModule)
    controller.getSignal('test')({ bar: 'bar' })
    controller.getSignal('test')({ bar: 'nope' })
    assert.deepEqual(results, ['true', 'false'])
  })
  test('should check truthy state using function and literal values', () => {
    const results = []
    const rootModule = Module({
      signals: {
        test: [
          when(props`foo`, 'bar', (foo, bar) => foo === bar),
          {
            true: [
              () => {
                results.push('true')
              },
            ],
            false: [
              () => {
                results.push('false')
              },
            ],
          },
        ],
      },
    })
    const controller = Controller(rootModule)
    controller.getSignal('test')({ foo: 'bar' })
    controller.getSignal('test')({ foo: 'nope' })
    assert.deepEqual(results, ['true', 'false'])
  })
  test('should fail on incomplete path definition', () => {
    const rootModule = Module({
      signals: {
        test: [when(props`foo`)],
      },
    })
    const controller = Controller(rootModule)
    assert.throws(() => {
      controller.getSignal('test')({ foo: 'bar' })
    })
  })
  test('should fail on incorrect usage', () => {
    const rootModule = Module({
      signals: {
        test: [when(false), { true: [], false: [] }],
      },
    })
    const controller = Controller(rootModule)
    assert.throws(() => {
      controller.getSignal('test')()
    })
  })
})
