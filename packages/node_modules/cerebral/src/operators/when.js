...
function whenFactory(...args) {
  ...
  function when({ state, path, resolve }) {
    if (!resolve.isResolvable(valueTemplates[0])) {
      throw new Error(
        `Cerebral operator.when: You have to a resolvable value, see: ${HELP_URL}`
      )
    }
    if (!path || !path.true || !path.false) {
      throw new Error(
        `Cerebral operator.when: true/false paths need to be provided, see: ${HELP_URL}`
      )
    }
