...
class BaseController extends FunctionTree {
  constructor(rootModule, options, functionTreeOptions) {
    ...
    this.contextProviders = Object.assign(
      ...
      getProviders(this.module),
      {
        ...
        operators:
          this.model.OperatorsProvider &&
          this.model.OperatorsProvider(this.devtools),
    ...
    if (isDeveloping()) {
      this.on('functionStart', (execution, functionDetails, payload) => {
        try {
          ...
          throwError(
            `The function ${functionDetails.name} in sequence ${
              execution.name
        ...
        (execution, functionDetails, payload, propsToAdd) => {
          if (devtools && devtools.preventPropsReplacement) {
            Object.keys(propsToAdd || {}).forEach((key) => {
              if (key in payload) {
                throw new Error(
                  `Cerebral Devtools - You have activated the "preventPropsReplacement" option and in sequence "${
                    execution.name
                  }", before the action "${
  ...
  */
  runSequence(name, sequence, payload = {}) {
    ...
    this.run(name, sequence, payload, (error) => {
      if (error) {
        ...
        if (catchingResult.catchingModule) {
          for (let [errorType, errorSequence] of catchingResult.catchingModule
            .catch) {
