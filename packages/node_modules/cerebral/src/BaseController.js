    ...
    if (isDeveloping()) {
      this.on('functionStart', (execution, functionDetails, payload) => {
        try {
          ...
          throwError(
            `The function ${functionDetails.name} in signal ${execution.name} is not given a valid payload`
          )
        }
      })
      this.on(
        ...
        (execution, functionDetails, payload, propsToAdd) => {
          if (devtools && devtools.preventPropsReplacement) {
            Object.keys(propsToAdd || {}).forEach(key => {
              if (key in payload) {
                throw new Error(
                  `Cerebral Devtools - You have activated the "preventPropsReplacement" option and in signal "${execution.name}", before the action "${functionDetails.name}", the key "${key}" was replaced`
                )
              }
            })
          }
          ...
          for (let [errorType, signalChain] of catchingResult.catchingModule
            ...
            if (error instanceof errorType) {
              ...
              if (this.throwToConsole) {
                setTimeout(() => {
                  console.log(
                    `Cerebral is handling error "${error.name}: ${error.message}" thrown by signal "${error
                      .execution.name}". Check debugger for more information.`
                  )
                })
              }
