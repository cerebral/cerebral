...
describe('DEPRECATED state', () => {
  it('should be able to GET state', () => {
    // Set up a temporary spy for console.warn
    const warnSpy = [];
    ...
    const rootModule = {
      ...
      sequences: {
        foo: [({ state }) => assert.deepStrictEqual(state.get(), { foo: 'bar' })],
      },
    }
    const controller = App(rootModule);
    controller.getSequence('foo')();

    // Restore console.warn and assert warning was logged
    console.warn = originalWarn;
    assert.ok(
      warnSpy.some(msg => msg.includes("DEPRECATED")),
