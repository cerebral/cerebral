...
describe('module', () => {
  ...
  it('should be able to SET state in subModule', () => {
    ...
    const controller = App(rootModule)
    controller.getSequence('subModule.foo')()
    assert.strictDeepEqual(controller.getState(), { subModule: { foo: 'bar2' } })
  })
    ...
    const controller = App(rootModule, {
      devtools: {
        ...
        sendExecutionData(data) {
          ...
          assert.strictDeepEqual(data.args[0], ['subModule', 'foo'])
        },
      },
    })
    controller.getSequence('subModule.foo')()
    assert.strictDeepEqual(controller.getState(), { subModule: { foo: ['bar'] } })
  })
})
