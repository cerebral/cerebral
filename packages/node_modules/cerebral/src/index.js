...
export default function App(rootModule, options) {
  if (
    ...
    process.env.NODE_ENV === 'production' ||
    (options && options.hotReloading === false)
  ) {
    return new ControllerClass(rootModule, options)
  }
  ...
  if (existingInstance && (!options || !options.hotReloading)) {
    console.warn(
      'Cerebral has detected HOT RELOADING in your app. If you want to remove this message set "hotReloading" to true, or false to turn off hot reloading completely'
    )
  }
