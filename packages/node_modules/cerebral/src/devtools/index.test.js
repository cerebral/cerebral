...
describe('Devtools', () => {
  ...
  test('should throw when host is not set', () => {
    assert.throws(
      () => {
  ...
  test('should init correctly and work when debugger is open when app loads', (done) => {
  ...
  test('should warn, try to reconnect to Debugger and send bulk message after connecting', (done) => {
    let warnCount = 0
  ...
  test('should set component details and watch executions', (done) => {
    const mockServer = new Server('ws://localhost:8585')
    let messages = {}
    let messageTypes = []
    mockServer.on('connection', (server) => {
  ...
  test('should watch signal execution error', (done) => {
    const mockServer = new Server('ws://localhost:8585')
    let messages = {}
    let messageTypes = []
    mockServer.on('connection', (server) => {
  ...
  test('should reset the state', (done) => {
    const mockServer = new Server('ws://localhost:8585')
    mockServer.on('connection', (server) => {
      server.on('message', (event) => {
        const message = JSON.parse(event)
  ...
  test('should warn when remember message sent if storeMutations option is false', (done) => {
    let warnCount = 0
    const originWarn = console.warn
    console.warn = function(...args) {
      warnCount++
  ...
  test('should travel back in time', (done) => {
    const mockServer = new Server('ws://localhost:8585')
    mockServer.on('connection', (server) => {
      server.on('message', (event) => {
        const message = JSON.parse(event)
  ...
  test('should warn when the signal fired while debugger is remembering state', (done) => {
    let warnCount = 0
    const originWarn = console.warn
    console.warn = function(...args) {
      warnCount++
  ...
  test('should change model state when debugger model state changed', (done) => {
    const mockServer = new Server('ws://localhost:8585')
    mockServer.on('connection', (server) => {
      server.on('message', (event) => {
        const message = JSON.parse(event)
