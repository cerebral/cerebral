...
export class Devtools extends DevtoolsBase {
  constructor({
    ...
    this.debuggerWatchMap = {}
    this.debuggerComputedMap = {}
    ...
    this.sendWatchMapBatch = debounce(this.sendWatchMapBatch, 50)
  }
  createSocket() {
  ...
  /*
    ...
    passes the execution id of the sequence that was double clicked. This
    ...
    prevent any new sequences firing off when in "remember state"
  */
  remember(index) {
    ...
    if (index === 0) {
      ...
      this.controller.run = (name) => {
        console.warn(
          `The sequence "${name}" fired while debugger is remembering state, it was ignored`
        )
      }
    }
    ...
    for (let x = 0; x < this.mutations.length - index; x++) {
      ...
      this.controller.model[method](...mutation.args)
    }
    ...
    const message = JSON.stringify({
      ...
      data: {
        initialModel: this.initialModelString
          ...
          : initialModel,
        initialComputedState: Object.keys(this.computedState).reduce(
          (withValue, path) => {
            withValue[path] = this.computedState[path].getValue()
  ...
  */
  ...
  updateComputedState(path, value) {
    if (!this.isConnected) {
      return
    }
  ...
  /*
    Manages computed registered to the state, allowing the devtools to update the
    debugger when values update
  */
  registerComputedState(computed, path) {
    this.computedState[path.join('.')] = computed
  }
  /*
  ...
  */
  updateWatchMap(watch, nextDeps, prevDeps) {
    if (prevDeps) {
      for (const depsKey in prevDeps) {
        if (nextDeps && depsKey in nextDeps) {
          continue
        }
        const debuggerWatchMapWatchList = this.debuggerWatchMap[depsKey]
        debuggerWatchMapWatchList.splice(
          debuggerWatchMapWatchList.indexOf(watch),
          1
        )
        if (debuggerWatchMapWatchList.length === 0) {
          delete this.debuggerWatchMap[depsKey]
        }
      }
    }
    ...
    if (nextDeps) {
      for (const depsKey in nextDeps) {
        if (prevDeps && depsKey in prevDeps) {
          continue
        }
        this.debuggerWatchMap[depsKey] = this.debuggerWatchMap[depsKey]
          ? this.debuggerWatchMap[depsKey].concat(watch)
          : [watch]
      }
    }
  }
  ...
  */
  updateComputedMap(computed, subscribers) {
    if (computed instanceof Compute) {
      this.debuggerComputedMap[computed.name] = subscribers
        .filter((subscriber) => subscriber.watcher !== computed)
        .map((subscriber) => subscriber.watcher)
    }
  }
  /*
    ...
    debugger updates
  */
  sendWatchMap(watchUpdates, changes, start, duration) {
    if (this.isConnected) {
