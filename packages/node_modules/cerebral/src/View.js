...
class View {
  constructor({
    ...
    this.dependencyMap = this.controller.createDependencyMap(
      Object.keys(dependencies).map((key) => dependencies[key]),
      props
    )

    this.computed = this.getComputedClones(props)
  }
  /*
  ...
  */
  ...
  mount() {
    const depsMap = Object.assign({}, this.dependencyMap)

    this.controller.dependencyStore.addEntity(this, depsMap)
  ...
  unMount() {
    const depsMap = Object.assign({}, this.dependencyMap)
    this.controller.dependencyStore.removeEntity(this, depsMap)
  ...
  */
  ...
  getComputedClones(props) {
    const getters = this.controller.createGetters(props)
    return Object.keys(this.dependencies).reduce((computeds, key) => {
      if (
        this.dependencies[key] instanceof Tag &&
        this.dependencies[key].type === 'computed'
      ) {
        const computed = this.dependencies[key].getValue(getters)
        computeds[key] = computed.propsTags.length ? computed.clone() : computed
        computeds[key].subscribe(this.updateComponent)
      }
  ...
  getProps(props = {}, includeProps = true) {
    ...
    const dependenciesProps = Object.keys(this.dependencies).reduce(
      (currentProps, key) => {
        ...
        if (tag.type === 'computed') {
          currentProps[key] = this.computed[key].getValue(props)
        } else {
    ...
    if (this.mergeProps) {
      return this.mergeProps(
        dependenciesProps,
        props,
        createResolver(this.controller.createGetters(props))
      )
    }
