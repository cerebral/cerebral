...
class View extends Watch {
  ...
  */
  ...
  mount() {
    ...
    this.computeds = this.getComputeds(this.props)
  }
  unMount() {
    Object.keys(this.computeds).forEach((key) => {
      if (this.computeds[key].computed.propsTags.length) {
        this.computeds[key].computed.destroy()
      } else {
        this.computeds[key].unsubscribe()
      }
    })
  ...
  getComputeds(props) {
    ...
    return Object.keys(this.dependencies).reduce((computeds, key) => {
      if (
        ...
        const attachedComputed = computed.propsTags.length
          ? computed.clone()
          : computed
        computeds[key] = {
  ...
  getProps(getters, props = {}, includeProps = true) {
    const dependenciesProps = Object.keys(this.dependencies).reduce(
      (currentProps, key) => {
        ...
        if (tag.type === 'computed') {
          currentProps[key] = this.computeds[key].computed.getValue(props)
        } else {
