...
class View {
  constructor({
    ...
    this.computedGetter = this.computedGetter.bind(this)
    ...
    */
    this.dependencyTrackers = Object.keys(dependencies).reduce(
      (currentDependencyTrackers, dependencyKey) => {
        ...
        } else if (dependencies[dependencyKey].type === 'computed') {
          const getters = this.createTagGetters(props)
          currentDependencyTrackers[dependencyKey] = new DependencyTracker(
            dependencies[dependencyKey].getValue(getters)
          )

          const modulePath = dependencies[dependencyKey]
            .getPath(getters)
            .split('.')
          modulePath.pop()
          currentDependencyTrackers[dependencyKey].run(
            this.stateGetter,
            props,
            modulePath.join('.')
          )
  ...
  */
  ...
  /*
    A getter for tags to grab computed of module from Cerebral
  */
  computedGetter(path) {
    return this.controller.getComputed(path)
  }
  /*
