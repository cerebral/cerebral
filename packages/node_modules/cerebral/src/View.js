  ...
  getComputed(props) {
    ...
    return Object.keys(this.dependencies).reduce((computed, key) => {
      if (
        ...
        isComputedValue(this.dependencies[key].getValue(getters))
      ) {
        ...
        var subscribeUp = (computedInstance) => {
          computedInstance.getValue(props)
          computed[key] = {
            computed: computedInstance,
            unsubscribe: computedInstance.subscribe(this, this.updateComponent),
          }

          if (computedInstance.computedTags)
            computedInstance.computedTags
              .map((tag) => tag.getValue(getters))
              .forEach(subscribeUp)
        }
  ...
  getProps(getters, props = {}, includeProps = true) {
    const dependenciesProps = Object.keys(this.dependencies).reduce(
      (currentProps, key) => {
        ...
        currentProps[key] = isComputedValue(value)
