...
describe('factory.increment', () => {
  it('should increment value in model with value of one by default', () => {
    ...
    assert.strictDeepEqual(controller.getState(), { foo: 1 })
  })
  it('should increment value in model with given value', () => {
    ...
    const controller = App(rootModule)
    controller.getSequence('test')()
    assert.strictDeepEqual(controller.getState(), { foo: 2 })
  })
    ...
    const rootModule = {
      sequences: {
        test: [
          increment(props`foo`),
          ({ props }) => {
            assert.strictEqual(props.foo, 1)
          },
        ],
      },
    }
    ...
    const rootModule = {
      sequences: {
        test: [
          ...
          ({ props }) => {
            assert.strictEqual(props.foo, 2)
          },
        ],
      },
    }
    ...
    const rootModule = {
      ...
      sequences: {
        test: [
          ...
          ({ props }) => {
            assert.strictEqual(props.value, 3)
          },
        ],
      },
    }
  ...
  it('should increment value to model from model', () => {
    ...
    const controller = App(rootModule)
    controller.getSequence('test')()
    assert.strictEqual(controller.getState().foo, 3)
  })
