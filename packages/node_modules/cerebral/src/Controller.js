...
class Controller extends BaseController {
  ...
  */
  ...
  updateDependencies(changes, force) {
    ...
    if (force) {
      dependenciesToUpdate = this.dependencyStore.getAllUniqueEntities()
    } else {
      dependenciesToUpdate = this.dependencyStore.getUniqueEntities(changes)
    }

    const start = Date.now()
    dependenciesToUpdate.forEach((dependency) => {
      if (this.devtools && dependency instanceof View) {
        this.devtools.updateComponentsMap(dependency)
      }
      dependency.onUpdate(changes, force)
    })
    const end = Date.now()

    if (this.devtools && dependenciesToUpdate.length) {
      const componentsToUpdate = dependenciesToUpdate.filter(
        (dependency) => dependency instanceof View
      )
      if (componentsToUpdate.length) {
        this.devtools.sendComponentsMap(componentsToUpdate, changes, start, end)
      }
    }
  }
  /*
  ...
  */
  createGetters(props, modulePath = []) {
    ...
    return {
      ...
      module: (path) => this.getState(modulePathString + path),
    }
  }
  /*
  ...
  */
  createDependencyMap(dependencies, props, modulePath) {
    const getters = this.createGetters(props, modulePath)
