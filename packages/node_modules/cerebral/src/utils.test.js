...
describe('utils', () => {
  ...
  describe('copyAndFreezeObject', () => {
    it('should not share references with the original', () => {
      ...
      const copy = utils.copyAndFreezeObject(original)
      assert.notEqual(copy, original)
      assert.notEqual(copy.foo, original.foo)
      assert.notEqual(copy.foo.bar, original.foo.bar)
    })
    it('should be equal in its graph', () => {
      const original = {
        foo: {
          bar: {
            bam: 1,
    ...
    it('should freeze all objects and arrays in tree', () => {
      const original = {
        foo: {
          bar: {
            bam: 1,
      ...
      assert(Object.isFrozen(copy.foo))
      assert(Object.isFrozen(copy.foo.bar))
      assert(Object.isFrozen(copy.foo.boo))
      assert(Object.isFrozen(copy.foo.boo.bag))
    })
  })
})
