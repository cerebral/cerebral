...
import App, { state, props, moduleState } from './'
...
describe('App', () => {
  ...
  it('should add subModule using path', () => {
    ...
    controller.addModule('foo.bar', moduleA)
  ...
  it('should resolve nested state with modulesState tag', () => {
    const main = {
      state: {},
      modules: {
        foo: {
          ...
          sequences: {
            test: [
              ({ get }) => {
                assert.strictEqual(get(state`foo.data.subItem`), 'value')
                assert.strictEqual(get(moduleState`data.subItem`), 'value')
              },
            ],
          },
        },
