...
class Module {
  ...
  create(controller, path) {
    ...
    /* Instantiate computeds */
    module.computeds = Object.keys(module.computeds || {}).reduce(
      (registered, computedKey) => {
        if (!(module.computeds[computedKey] instanceof Computed)) {
          throw new Error(
            `You are not using a Computed in module on key "${computedKey}"`
          )
        }

        registered[computedKey] = module.computeds[computedKey].create(
          controller,
          path
        )
    ...
    /* Instantiate watcher */
    module.watchers = Object.keys(module.watchers || {}).reduce(
      (registered, watcherKey) => {
        if (!(module.watchers[watcherKey] instanceof Watcher)) {
          throw new Error(
            `You are not using a Watcher in module on key "${watcherKey}"`
          )
        }

        registered[watcherKey] = module.watchers[watcherKey].create(
          controller,
          path
        )
