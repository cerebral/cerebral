...
describe('Compute', () => {
  it('should Compute a value', () => {
    ...
    assert.strictEqual(controller.get(state`foo`), 'foo')
  })
  it('should cache value', () => {
    ...
    const controller = App(main)
    assert.strictEqual(controller.get(state`foo`), 0)
    assert.strictEqual(controller.get(state`foo`), 0)
  })
  it('should Compute state', () => {
    ...
    const controller = App(main)
    assert.strictEqual(controller.get(state`fooCopy`), 'bar')
  })
  it('should update Compute when state changes', () => {
    ...
    assert.strictEqual(controller.get(state`fooCopy`), 'bar2')
  })
  ...
  it('should Compute module state with submodule', () => {
    const main = {
      state: {},
      modules: {
        foo: {
          state: {
            bar: 'baz',
            barCopy: (get) => get(moduleState`bar`),
          },
        },
      },
    }
    const controller = App(main)
    assert.strictEqual(controller.get(state`foo.barCopy`), 'baz')
  })
  ...
  it('should Compute props', () => {
    ...
    const app = App(main)
    assert.strictEqual(app.get(state`foo`, { foo: 'bar' }), 'bar')
  })
  it('should Compute other computed state', () => {
    ...
    const controller = App(main)
    assert.strictEqual(controller.get(state`bar`), 'foo')
  })
  it('should Compute when computed state updates', () => {
    ...
    const controller = App(main)
    assert.strictEqual(controller.get(state`bar`), 'bar')
    controller.getSequence('updateFoo')()
    assert.strictEqual(controller.get(state`bar`), 'bar2')
  })
  it('should allow getting path with get', () => {
    ...
    const controller = App(main)
    assert.strictEqual(controller.get(state`foo`), 'foo.bar')
  })
  ...
  it('should throw when trying to replace computed in nested path', () => {
    const main = {
      state: {
        foo: {
          bar: (get) => get.path(state`foo.bar`),
        },
      },
      sequences: {
        test: set(state`foo`, 'bar'),
