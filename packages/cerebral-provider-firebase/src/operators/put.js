...
function putFactory (putPath, file, options = {}) {
  function put ({firebase, props, state, path, resolve}) {
    const evaluatedOptions = Object.keys(
      options
    ).reduce((currentEvaluatedOptions, key) => {
      const option = options[key]

      if (key === 'progress') {
        if (!resolve.isTag(option, 'state', 'signal')) {
          throw new Error(
            "cerebral-module-firebase: The value for 'progress' option should be either 'state' or 'signal' tag."
          )
        }
