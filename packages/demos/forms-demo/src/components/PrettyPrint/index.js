...
export default connect(
  ...
  function PrettyPrint({ form, showPanel }) {
    ...
    let result = form.isValid
      ? form.toJSON()
      : Object.keys(form.getInvalidFields()).reduce((acc, field) => {
    ...
    const resultPane = css(form.isValid ? styles.successPane : styles.errorPane)
    return (
      <div className={css(styles.container)}>
        <div className={resultPane}>
          {form.isValid
            ? 'The form is valid'
            : 'The form is invalid. See invalid fields below'}
        </div>
        <div className={css(styles.innerContainer)}>
          <pre
            className={css(styles.pretty)}
            dangerouslySetInnerHTML={{
              __html: syntaxHighlight(JSON.stringify(result, undefined, 2)),
            }}
          />
        </div>
      </div>
